...
class SpoolDirCsvSourceConnectorConfig extends SpoolDirSourceConnectorConfig {
  ...
  static final String CSV_ESCAPE_CHAR_DOC =
  ...
  static final String CSV_IGNORE_LEADING_WHITESPACE_DOC =
  ...
  static final String CSV_NULL_FIELD_INDICATOR_DOC = "Indicator to determine how the CSV Reader can determine if a field is null. Valid values are "
      + ConfigUtils.enumValues(CSVReaderNullFieldIndicator.class)
  ...
  static final String CSV_CASE_SENSITIVE_FIELD_NAMES_DOC =
  ...
  private static final String CSV_QUOTE_CHAR_DOC =
  ...
  public SpoolDirCsvSourceConnectorConfig(final boolean isTask, Map<String, ?> settings) {
    ...
    this.nullFieldIndicator =
        ConfigUtils.getEnum(CSVReaderNullFieldIndicator.class, this, SpoolDirCsvSourceConnectorConfig.CSV_NULL_FIELD_INDICATOR_CONF);
  ...
  static final ConfigDef conf() {
    ...
    return SpoolDirSourceConnectorConfig.config()
        .define(ConfigKeyBuilder.of(CSV_SKIP_LINES_CONF, ConfigDef.Type.INT).defaultValue(CSV_SKIP_LINES_DEFAULT).importance(ConfigDef.Importance.LOW)
            .documentation(CSV_SKIP_LINES_DOC).group(CSV_GROUP).width(ConfigDef.Width.LONG).displayName(CSV_SKIP_LINES_DISPLAY).build())
        .define(ConfigKeyBuilder.of(CSV_SEPARATOR_CHAR_CONF, ConfigDef.Type.INT).defaultValue(CSV_SEPARATOR_CHAR_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_SEPARATOR_CHAR_DOC).group(CSV_GROUP).width(ConfigDef.Width.LONG)
            .displayName(CSV_SEPARATOR_CHAR_DISPLAY).build())
        .define(ConfigKeyBuilder.of(CSV_QUOTE_CHAR_CONF, ConfigDef.Type.INT).defaultValue(CSV_QUOTE_CHAR_DEFAULT).importance(ConfigDef.Importance.LOW)
            .documentation(CSV_QUOTE_CHAR_DOC).group(CSV_GROUP).width(ConfigDef.Width.LONG).displayName(CSV_QUOTE_CHAR_DISPLAY).build())
        .define(
            ConfigKeyBuilder.of(CSV_ESCAPE_CHAR_CONF, ConfigDef.Type.INT).defaultValue(CSV_ESCAPE_CHAR_DEFAULT).importance(ConfigDef.Importance.LOW)
                .documentation(CSV_ESCAPE_CHAR_DOC).group(CSV_GROUP).displayName(CSV_ESCAPE_CHAR_DISPLAY).width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_STRICT_QUOTES_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_STRICT_QUOTES_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_STRICT_QUOTES_DOC).displayName(CSV_STRICT_QUOTES_DISPLAY).group(CSV_GROUP)
            .width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_IGNORE_LEADING_WHITESPACE_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_IGNORE_LEADING_WHITESPACE_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_IGNORE_LEADING_WHITESPACE_DOC).group(CSV_GROUP)
            .displayName(CSV_IGNORE_LEADING_WHITESPACE_DISPLAY).width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_IGNORE_QUOTATIONS_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_IGNORE_QUOTATIONS_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_IGNORE_QUOTATIONS_DOC).displayName(CSV_IGNORE_QUOTATIONS_DISPLAY).group(CSV_GROUP)
            .width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_KEEP_CARRIAGE_RETURN_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_KEEP_CARRIAGE_RETURN_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_KEEP_CARRIAGE_RETURN_DOC).displayName(CSV_KEEP_CARRIAGE_RETURN_DISPLAY)
            .group(CSV_GROUP).width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_VERIFY_READER_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_VERIFY_READER_DEFAULT)
            .importance(ConfigDef.Importance.LOW).documentation(CSV_VERIFY_READER_DOC).group(CSV_GROUP).displayName(CSV_VERIFY_READER_DISPLAY)
            .width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_NULL_FIELD_INDICATOR_CONF, ConfigDef.Type.STRING).defaultValue(CSV_NULL_FIELD_INDICATOR_DEFAULT)
            .validator(ValidEnum.of(CSVReaderNullFieldIndicator.class)).importance(ConfigDef.Importance.LOW)
            .documentation(CSV_NULL_FIELD_INDICATOR_DOC).displayName(CSV_NULL_FIELD_INDICATOR_DISPLAY).group(CSV_GROUP).width(ConfigDef.Width.LONG)
            .build())
        .define(ConfigKeyBuilder.of(CSV_FIRST_ROW_AS_HEADER_CONF, ConfigDef.Type.BOOLEAN).defaultValue(CSV_FIRST_ROW_AS_HEADER_DEFAULT)
            .importance(ConfigDef.Importance.MEDIUM).documentation(CSV_FIRST_ROW_AS_HEADER_DOC).displayName(CSV_FIRST_ROW_AS_HEADER_DISPLAY)
            .group(CSV_GROUP).width(ConfigDef.Width.LONG).build())
        .define(ConfigKeyBuilder.of(CSV_CHARSET_CONF, ConfigDef.Type.STRING).defaultValue(CSV_CHARSET_DEFAULT).validator(CharsetValidator.of())
            .importance(ConfigDef.Importance.LOW).documentation(CSV_CHARSET_DOC).displayName(CSV_CHARSET_DISPLAY).group(CSV_GROUP)
            .width(ConfigDef.Width.LONG).build())
        .define(
            ConfigKeyBuilder.of(CSV_CASE_SENSITIVE_FIELD_NAMES_CONF, ConfigDef.Type.BOOLEAN).defaultValue(false).importance(ConfigDef.Importance.LOW)
                .documentation(CSV_CASE_SENSITIVE_FIELD_NAMES_DOC).displayName(CSV_CASE_SENSITIVE_FIELD_NAMES_DISPLAY).build());
  }
  ...
  public CSVParserBuilder createCSVParserBuilder() {
    ...
    return new CSVParserBuilder().withEscapeChar(this.escapeChar).withIgnoreLeadingWhiteSpace(this.ignoreLeadingWhitespace)
        .withIgnoreQuotations(this.ignoreQuotations).withQuoteChar(this.quoteChar).withSeparator(this.separatorChar)
        .withStrictQuotes(this.strictQuotes).withFieldAsNull(nullFieldIndicator);
  }

  public CSVReaderBuilder createCSVReaderBuilder(Reader reader, CSVParser parser) {
    return new CSVReaderBuilder(reader).withCSVParser(parser).withKeepCarriageReturn(this.keepCarriageReturn).withSkipLines(this.skipLines)
        .withVerifyReader(this.verifyReader).withFieldAsNull(nullFieldIndicator);
  }
